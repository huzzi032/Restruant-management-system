...
api.interceptors.response.use(
  ...
  (error) => {
    if (error.response?.status === 401) {
      ...
      // Never force-logout for these cases:
      // 1. Login/signup — they naturally return 401 on bad credentials
      // 2. /auth/me — used for token validation on mount; handled by AuthContext directly
      // 3. Any request explicitly flagged to skip auth expiry handling
      ...
      if (!isAuthEndpoint && !skipExpiry && !isLoggingOut && tokenExists) {
        ...
        // Reset the debounce flag after a longer delay to prevent cascading logouts
