      ...
      <Card className={`border-l-4 ${config.border} transition-shadow hover:shadow-md`}>
        ...
        <CardContent>
          ...
          <div className="flex gap-2 mt-4">
            <Button
              className={`flex-1 h-12 text-base font-semibold transition-all ${buttonConfig.className}`}
            ...
            <Button
              variant="outline"
              className="h-12 w-12 border-destructive text-destructive hover:bg-destructive hover:text-destructive-foreground shrink-0"
...
export default function KitchenDisplay() {
  ...
  const handleAction = (order_id: number, status: string) => {
    setPendingOrderId(order_id);
    if (status === 'in_kitchen') {
      startCookingMutation.mutate(order_id);
    } else if (status === 'cooking') {
      markReadyMutation.mutate(order_id);
    } else if (status === 'ready') {
      pickupMutation.mutate(order_id);
    }
  };
  ...
  const waiting = filteredOrders?.filter((o) => o.status === 'in_kitchen') || [];
