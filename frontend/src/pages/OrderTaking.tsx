          ...
          <CardContent className="p-4">
            ...
            <div className="flex flex-col gap-3 mb-4">
              <div className="flex items-center justify-between">
                <p className="text-sm font-medium text-muted-foreground uppercase tracking-wider">Modify Active Orders</p>
                <Badge variant="secondary">{activeOrders?.length || 0} active</Badge>
              </div>
            ...
            <div className="flex flex-wrap gap-2">
              {filteredActiveOrders?.length ? filteredActiveOrders.map((order) => (
                <Button
                  key={order.id}
                ...
                <p className="text-sm text-muted-foreground">No matching active orders</p>
              )}
            </div>
        ...
        <div className="border-t p-4 space-y-4">
          ...
          <div className="flex gap-2">
            <Button
              variant="destructive"
              className="flex-1"
              disabled={!selectedOrderToEdit || cancelMutation.isPending}
              onClick={() => cancelMutation.mutate(selectedOrderToEdit!)}
            >
            ...
            >
              {addItemsToOrderMutation.isPending ? 'Updating Order...' : 'Add To Order'}
            </Button>
          </div>
        </div>
      </Card>
